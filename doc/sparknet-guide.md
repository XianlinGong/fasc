...
(note, if using the debug window in the Fabcoin Qt application, don't include `./fabcoin-cli` in the commands)

First, we need to deploy the contract:

    ./fabcoin-cli createcontract  300000
...
Now wait for your contract to be included in a block. You should be able to confirm it made it into a block by using:
    ...
    ./fabcoin-cli getaccountinfo $CONTRACT
...
Now, because we are not changing state, we use `callcontract`:
    ...
    ./fabcoin-cli callcontract $CONTRACT 3450bd6a
...
Note we use --lenient so that we do not need to provide a full 256bit value as a parameter. Now, we can execute the contract directly:
    ...
    ./fabcoin-cli sendtocontract $CONTRACT 3fb5c1cb000000000000000000000000000000000000000000000000000000000001e240
...
This will send 10 tokens to the contract:
    ...
    ./fabcoin-cli sendtocontract $CONTRACT d0e30db0 10

And then, to withdraw them it's also very simple:

    ./fabcoin-cli sendtocontract $CONTRACT 3ccfd60b
...
If you want to control the exact address that the contract sends coins to, you can also explicitly specify the sender. Note that you must be capable of sending coins from that address (you can't use someone elses address). One of my wallet addresses is qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt, so I'll use that:
    ...
    ./fabcoin-cli sendtocontract $CONTRACT 3ccfd60b 0 190000 0.0000001 qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt
...
Note that if you get the error "Sender address does not have any unspent outputs", then you should send some coins to that address (they must be spent in order to prove that you own that address). This can be accomplished with any amount of coins:
    ...
    ./fabcoin-cli sendtoaddress qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt 0.001
...
* Q: "I think I found a bug in Fabcoin" A: Please report any bugs at https://github.com/blockchaingate/fabcoin/issues



# New Fabcoin RPC Commands

Fabcoin supports all of the RPC commands supported by Fabcoin Core, but also includes the following commands unique to Fabcoin:

* `createcontract` - This will create and deploy a new smart contract to the Fabcoin blockchain. This requires gas.
* `callcontract` - This will interact with an already deployed smart contract on the Fabcoin blockchain, with all computation taking place off-chain and no persistence to the blockchain. This does not require gas
* `sendtocontract` - This will interact with an already deployed smart contract on the Fabcoin blockchain. All computation takes place on-chain and any state changes will be persisted to the blockchain. This allows tokens to be sent to a smart contract. This requires gas.
...
# New Fabcoin Command Line Arguments

Fabcoin supports all of the usual command line arguments that Fabcoin Core supports. In addition it adds the following new command line arguments:

* `-record-log-opcodes` - This will create a new log file in the Fabcoin data directory (usually ~/.fabcoin) named vmExecLogs.json, where any EVM LOG opcode is logged along with topics and data that the contract requested be logged. 

# Untested features

Some features included in Fabcoin Core have not been tested in it's porting to Fabcoin. This includes:

* Pruning

# EVM Smart Contract Changes and Limitations

Because of Fabcoin's underlying technical differences, there are a few operations that can have different results or limitations when executed in Fabcoin than when compared to Ethereum. 

These include the following, though there may be others introduced in the future:  

* The gas schedule for Fabcoin is different from Ethereum. Certain operations are more or less expensive. As such, gas cost estimators designed for Ethereum will not give accurate results for Fabcoin. We will develop our own gas estimating tools as well as fully documenting these differences at a later date. 
...
Additional documents for the overall design and expected results of various operations is available at the ITD repository here: https://github.com/blockchaingate/fabcoin-itds
