...
QString TokenTransactionDesc::toHTML(CWallet *wallet, CTokenTx &wtx, TokenTransactionRecord *rec)
{
    ...
    if(nCredit > 0)
    {
         strHTML += "<b>" + tr("Credit") + ":</b> " + FabcoinUnits::formatTokenWithUnit(unit, rec->decimals, nCredit, true) + "<br>";
    }
    if(nDebit < 0)
    {
         strHTML += "<b>" + tr("Debit") + ":</b> " + FabcoinUnits::formatTokenWithUnit(unit, rec->decimals, nDebit, true) + "<br>";
    }
    strHTML += "<b>" + tr("Net Amount") + ":</b> " + FabcoinUnits::formatTokenWithUnit(unit, rec->decimals, nNet, true) + "<br>";
