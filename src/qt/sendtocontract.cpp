    ...
    m_results(1)
{
    ...
    ui->labelAmount->setToolTip(tr("The amount in FABCOIN to send. Default = 0."));
...
void SendToContract::on_sendToContractClicked()
{
    if(isDataValid())
    {
        ...
        QString amount = isFunctionPayable() ? FabcoinUnits::format(unit, ui->lineEditAmount->value(), false, FabcoinUnits::separatorNever) : "0";
