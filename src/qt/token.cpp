...
{
    CTxDestination fabTokenAddress = DecodeDestination(strFabTokenAddress);
    if(!IsValidDestination(fabTokenAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&fabTokenAddress);
    if(keyid){
...
bool ToFabTokenAddress(const std::string& strHash160, std::string& strFabTokenAddress)
{
    ...
    CKeyID keyid(key);
    CTxDestination fabTokenAddress = keyid;
    if(IsValidDestination(fabTokenAddress)){
        strFabTokenAddress = EncodeDestination(fabTokenAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToFabTokenAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToFabTokenAddress(tokenEvent.receiver, tokenEvent.receiver);
