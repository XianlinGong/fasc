...
{
    return QCoreApplication::translate("fabcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. fabcoin_de.qm (shortcut "de" needs to be defined in fabcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. fabcoin_de_DE.qm (shortcut "de_DE" needs to be defined in fabcoin.qrc)
 ...
 */
class FabcoinCore: public QObject
{
    Q_OBJECT
public:
    explicit FabcoinCore();
...
class FabcoinApplication: public QApplication
{
    Q_OBJECT
public:
    explicit FabcoinApplication(int &argc, char **argv);
    ~FabcoinApplication();
...
FabcoinApplication::FabcoinApplication(int &argc, char **argv):
    ...
    returnValue(0)
{
    ...
    // This must be done inside the FabcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", FabcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void FabcoinApplication::createPaymentServer()
{
...
{
    window = new FabcoinGUI(platformStyle, networkStyle, 0);
...
void FabcoinApplication::startThread()
{
    ...
    FabcoinCore *executor = new FabcoinCore();
...
void FabcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        if (!vpwallets.empty())
        {
            ...
            window->addWallet(FabcoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(FabcoinGUI::DEFAULT_WALLET);
        ...
        // fabcoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", FabcoinGUI::tr("A fatal error occurred. Fabcoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(fabcoin);
    Q_INIT_RESOURCE(fabcoin_locale);

    FabcoinApplication app(argc, argv);
    ...
    // fabcoin: links repeatedly have their payment requests routed to this process:
