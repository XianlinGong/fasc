...
#include <qt/fabcoinaddressvalidator.h>
#include <uint256.h>
    ...
    m_selectedToken(0)
{
    ...
    ui->lineEditPayTo->setCheckValidator(new FabcoinAddressCheckValidator(parent, true));
}
...
void SendTokenPage::on_confirmClicked()
{
    ...
    if(m_model && m_model->isUnspentAddress(m_selectedToken->sender))
    {
        ...
        m_tokenABI->setGasPrice(FabcoinUnits::format(unit, gasPrice, false, FabcoinUnits::separatorNever).toStdString());

        std::string toAddress = ui->lineEditPayTo->text().toStdString();
        std::string amountToSend = ui->lineEditAmount->text().toStdString();
        QString amountFormated = FabcoinUnits::formatToken(m_selectedToken->decimals, ui->lineEditAmount->value(), false, FabcoinUnits::separatorAlways);
