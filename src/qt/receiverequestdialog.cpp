...
{
#ifdef USE_QRCODE
    QString uri = GUIUtil::formatFabcoinURI(_info);
    label->setText("");
    if(!uri.isEmpty())
    {
        // limit URI length
...
void ReceiveRequestDialog::update()
{
    ...
    QString uri = GUIUtil::formatFabcoinURI(info);
    ...
    if(info.amount)
        html += tr("Amount")+": <font color='#ffffff'>" + FabcoinUnits::formatHtmlWithUnit(model->getDisplayUnit(), info.amount) + "</font><br>";
...
{
    GUIUtil::setClipboard(GUIUtil::formatFabcoinURI(info));
}
