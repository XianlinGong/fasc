...
TestChain800Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction>& txns, const CScript& scriptPubKey)
{
    ...
    while (!CheckProofOfWork(block.GetHash(), block.nBits, chainparams.GetConsensus(), false)) 
...
TestChain100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction>& txns, const CScript& scriptPubKey)
{
    ...
    while (!CheckProofOfWork(block.GetHash(), block.nBits, chainparams.GetConsensus(), false)) //++block.nNonce;
        block.nNonce = ArithToUint256(UintToArith256(block.nNonce) + 1);
