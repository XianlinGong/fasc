...
void initState(){
    ...
    pathTemp = fs::temp_directory_path() / strprintf("test_fabcoin_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
    boost::filesystem::create_directories(pathTemp);
    const std::string dirFabcoin = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<FabcoinState>(new FabcoinState(dev::u256(0), FabcoinState::openDB(dirFabcoin, hashDB, dev::WithExisting::Trust), dirFabcoin + "/fabcoinDB", dev::eth::BaseState::Empty));
...
FabcoinTransaction createFabcoinTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    FabcoinTransaction txEth;
    if(recipient == dev::Address()){
        txEth = FabcoinTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = FabcoinTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
