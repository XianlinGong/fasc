...
void checkResult(bool isCreation, std::vector<FabcoinTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    FabcoinTxConverter converter(transaction, NULL);
    ExtractFabcoinTX fabcoinTx;
    BOOST_CHECK(converter.extractionFabcoinTransactions(fabcoinTx));
    std::vector<FabcoinTransaction> result = fabcoinTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(fabcointxconverter_tests, TestingSetup)
