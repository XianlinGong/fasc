...
class FabcoinState : public dev::eth::State {
    
public:

    FabcoinState();

    FabcoinState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, FabcoinTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
...
struct TemporaryState{
    std::unique_ptr<FabcoinState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<FabcoinState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(FabcoinState* _state, const std::vector<TransferInfo>& _transfers, const FabcoinTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const FabcoinTransaction& transaction;
