...
The RPM spec file provided here is for Fabcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://fabcoins.info/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Fabcoin git repository](https://github.com/blockchaingate/fabcoin) but are not part of
...
Currently only `Source10` is used, for the example `fabcoin.conf` file.
...
in the fabcoin git repository. At present only an SVG version of the Fabcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the fabcoin
...
needed when 0.12.1 is released, a proper fix is already in the Fabcoin git
master branch.
...
idea to build Fabcoin Core against a static version of that library compiled
...
The `fabcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `fabcoin-server` package. At this point, most RPM based
...
The `fabcoin-server` package should have SELinux support. How to properly do
...
Fabcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/fabcoin/). He has been
packaging Fabcoin for Fedora at least since 2012.
...
4. I renamed the `fabcoin` package that contains the Qt GUI to `fabcoin-core` as
that appears to be how the general population refers to it, in contrast to
`fabcoin-xt` or `fabcoin-classic`. I wanted to make sure the general population
