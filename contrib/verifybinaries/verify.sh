...
###   fabcoincore.org and fabcoins.info and compares them.
...
WORKINGDIR="/tmp/fabcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://fabcoincore.org"
HOST2="https://fabcoins.info"
BASEDIR="/bin/"
VERSIONPREFIX="fabcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'fabcoin-',
...
if [ $? -ne 0 ]; then
   echo "fabcoins.info failed to provide signature file, but fabcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "fabcoins.info and fabcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Fabcoin Core binary release signing key installed?"
   fi
